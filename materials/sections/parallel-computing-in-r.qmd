...
With `furrr`, you can use the `map()` pattern to execute an expression across a set of inputs using asynchronous futures. Mmost of the details here are hidden under the hood, so this usage pattern should feel really familiar to `purrr` users. For example, let's return to our bootstrap linear model, and reimplement it, first using purrr, and then with furrrr.
