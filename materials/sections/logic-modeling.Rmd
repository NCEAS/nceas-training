...
In a more detailed logic model, outputs and outcomes are further broken down. Outputs are often represented as 'Activities' and 'Participation'. By including participation (or participants), the logic model is explicitly considering the intended audience or stakeholders impacted by the program. Engagement of this audience is an outcome. Outcomes can be split into short, medium and long-term outcomes. Sometimes this last category may be labeled 'Impact'
...
![](images/LM4.png)
The above example includes two sections within 'Outputs' and this is quite common. You will find a number of different iterations of logic model templates, including versions with a third type of output - "Products'. Sometimes these products are contained within the 'Activities' section - for example, 'develop curricula', 'produce a report' - however calling these out explicitly is beneficial for teams focused on developing a product.

![](images/LM5.png)
Program development (and logic modeling) occurs in response to a given 'Situation' or need. This situation provides foundation for the objective, or problem, that the program is designed to solve. Hence some logic models may be framed with Problem and Solution statements. Finally, comprehensive logic modeling takes into consideration assumptions that are made with respect to the resources available, the people involved, the way the program will work etc and also recognizes that there are external factors that can impact the program's success.

![](images/LM6.png)

In summary:

A logic model comprises three primary steps in the workflow:
...
LM: What short-term outcomes are anticipated among participants. These can include as changes in awareness, knowledge, skills, attitudes, opinions and intent. 
...
- Logic model template on Sharepoint
