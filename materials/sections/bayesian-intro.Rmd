...
A corollary to the direct quantification of uncertainty is that the purpose of the analysis can be on estimation or parameters, rather than strict hypothesis testing. Bayesian inference provides a way to quantitatively describe relationships in complex datasets, which allows for inquiry to be driven by questions and models of understanding, rather than solely falsifiable hypotheses. 
...
In practice, selecting priors can be part of the model development process. Different priors can be tested for their contribution to the posterior, which allows for evaluation of the sensitivity of the posterior to the prior specification. Particulal for non-identifiable parameters, convergence can be achieved more quickly with a more informative prior. One method can be to specify a uniform prior with realistic but conservative upper and lower bounds:

$\theta \sim Uniform(A, B)$

As such, the posterior distribution for $\theta$ will also be restricted to the interval [A,B], but the data likelihood will contribute to its shape. 

## Graphical representations of hierarchical models

Complex or hierarchical models can be easily represented as a graphical model, particularly a Directed Acycling Graph (DAG). General convention for DAGs in Bayesian modeling are:
...
The conditional probability model for the above DAG is:

$P(A, B, C) = P(C|B)P(B|A)P(A)$

We will see in the example below that DAGs can be especially useful for helping translate between mathematical and code representations of models.
...
The hyperparameters $\alpha$ and $\beta$ are root nodes, whose distributions must be defined by numbers.
...
All of the stochastic nodes (ovals) are drawn from distributions. $\alpha$ and $\beta$ are root nodes, meaning that the parameter values for their distributions must be defined numerically. 

#### Interactive problem: Logistic regression example {- .aside}

Consider the case of selection pressure from discoloring of tree trunks from air pollution. One species of moth rests on tree trucks during the day, and their coloration acts as camouflage against bird predation. Researchers established a gradient of tree trunk color from Liverpool the countryside and selected 7 sites. At each site, they glued an equal number of dead light and dead dark moths to tree trunks and counted the number of removed moths after 24 hours. 
...
Consider the following questions:
 ...
 - Let $y$ represent the number of removed moths. What kind of likelihood should be given to this response variable?
 
 - The probability $p$ of removal differs depending on distance from Liverpool. Thus, we should index it as $p_i$. Assuming $\beta_1$ and $\beta_2$ as the regression parameters, how can $p_i$ be modeled in terms of $x_i$?
 
 - Furthermore, we hypothesize that color morph influences the relationship with distance. By turning $\beta_1$ and $\beta_2$ into vectors, can we account for different regression parameters based on the morph of observation $i$?
