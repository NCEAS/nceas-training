...
<!-- NOT WORKING CHECK **Line and point**

```{r line_poin_plot}
daily_visits_loc %>%
    separate(date, c("year", "month", "day"), sep = "-") %>%
    mutate(month = as.numeric(month)) %>% 
    filter(restore_loc == "Wildlands") %>%
    ggplot(aes(x = month, y = daily_visits)) +
    geom_line() +
    geom_point()
```
-->
