...
To install R, visit [cloud.r-project.org](https://cloud.r-project.org/) to download the most recent version for your operating system. The latest release is version 4.4.3 (2025-02-28, Trophy Case).
...
- **Update:** If you already have RStudio and need to update: Open RStudio, and under ‘Help’ in the top menu, choose ‘Check for updates.’ If you have the most recent release, it will return ‘No update available. You are running the most recent version of RStudio.’ Otherwise, you should follow the instructions to install an updated version (Note you'll have to download the new version in order to update).
...
- In the Terminal tab ([what is the Terminal?](https://learning.nceas.ucsb.edu/2024-working-groups/session_08.html#what-is-the-terminal)), run the following command:

```{bash eval=FALSE}
which git
```

- If after running that you get something that looks like a file path to git on your computer, then you have git installed. For example, that might return something like this (or it could differ a bit): `/usr/local/bin/git`. If you instead get no response at all or see something like `git command not found` or "git is not installed", you should download & install git from here: [git-scm.com/downloads](https://git-scm.com/downloads)


**An aside:** It is necessary to have Git installed in your machine for this workshop. GitHub's [Git Guides](https://github.com/git-guides) are a really wonderful resource to start learning about this topic.
...
:::callout-important
## Need help?
If you run into any issues during this process please check out [Happy Git nd GitHub for the useR](https://happygitwithr.com/) (great resource from Jennifer Bryan) or more details or contact us to help you troubleshoot.
...
- In the Terminal ([what is the Terminal?](https://learning.nceas.ucsb.edu/2024-working-groups/session_08.html#what-is-the-terminal)), run the following command:
...
### 2. Strategy to integrate changes from one branch into another{.unnumbered}
...
### 3. What is the terminal?{.unnumbered}
