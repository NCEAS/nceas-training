...
The text is read in as a list of strings, where each page of text is an item in the list.

To help us work with the rest of the `tidytext` tools, we use the `tibble::enframe` function, which converts the list into a `data.frame`, then rename the column to make it clear what page it is on.
...
Finally, we can try the same analysis again to look for the most commonly used words.
