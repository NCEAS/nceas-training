...
```{r}
#| echo: false
path_df <- "data/text/dsc-plan-ch3.pdf"
dp_ch3 <- pdftools::pdf_text(path_df)

path_df <- "data/text/dsc-plan-ch4.pdf"
dp_ch4 <- pdftools::pdf_text(path_df)
```
...
Tidy the data using word tokens, remove stop words, and combine both
into one dataframe.
:::
...
What _are_ these two topics, as expressed using the words most strongly associated with each one?
:::
