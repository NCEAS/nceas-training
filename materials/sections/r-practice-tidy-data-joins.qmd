...
**Note:** It's up to you on how you want to download and load the data! You can either use the datasets links (obtain by right-clicking the Download button and select "Copy Link Address" for each data entity) or manually download the data and then upload the files to RStudio server.
...
What are the primary and foreign keys for each datasets? Recall that a **primary key** is a unique identifier for each observed entity, one per row. And a **foreign key** references to a primary key in another table (linkage).
...
-   `sites`: Primary key is `site_id` and does not have a foreign key.
-   `surveys`: Primary key is `survey_id` and foreign key is `site_id`.
...
I expect to see all columns and all observations from `birds_subset` and all columns and all observations from `sites` to be merged into one dataset because in a full join everything is kept.
