...
<!--- UPDATE AND IMPROVE QUALITY OF IMAGES -->
<!-- ADD image form video to show Rmd structure. Add langueage round YAML -->
...
<!--- Clarify each of this Markdown syntax -->
...
<!--- IS THIS THE LATES VERSION?? -->

<!--- Ask Jeanette about introducing the "Visual editor" here -->

<!-- Re write using "editing mode"-->
...
![](images/markdown-citation.png) After selecting insert, a couple of things happen. First, the citation reference is inserted into your markdown text as `[@oke2020]`. Second, a file called `references.bib` containing the BibTex format of the citation is created. Third, that file is added to the YAML header of your RMarkdown document (`bibliography: references.bib`). Adding another citation will automatically update your `references.bib` file. So easy!
...
<!--- Think about introducing the librarian package? It installs necessary and loads your packages --->
...
RMarkdown has a special way of handling relative paths that can be very handy. When working in an RMarkdown document, R will set all paths relative **to the location of the RMarkdown file.** This way, you don't have to worry about setting a working directory, or changing your colleagues absolute path structure with the correct user name, etc. _If your RMarkdown is stored near where the data it analyses are stored (good practice, generally)_, setting paths becomes much easier!
...
<!--- Should we introduce to the different class objects?---> 
...
<!--- ADDING THIS SECTION -->
Let's start by exploring the data we just read. Every time I read a new data set, I like to familiarize my self with it and make sure that the data looks as expected. There are a couple of functions I use to do this. So we will create a new R chunk and run the following functions. Because this just an exploration and we do not want this chunc to be part of our report, we will indicate that by adding `eval = FALSE` in the setup of the chunk, that way, the code in this chunk will not run when I knit the final document.
...
<!--- explain that you can access the different columns in your data frame using the $ -->
...
-   [What is RMardown Video](https://vimeo.com/178485416)
