...
Commenting out code you don't want to lose is something probably all of us have done at one point or another but it's really hard to understand why you did this when you come back years later or when you send your script to a colleague.
...
Branches allow us to confidently experiment on our code, all while leaving the old code intact and recoverable.
...
If you weren't using a tool like `Git`, you might panic at this point because you've rewritten much of your analysis to use a different method but your boss wants to change to the old method.
...
Another really powerful feature of `Git` is _tags_ which allows us to record a particular state of our analysis with a meaningful name.
In this case, we are lucky because we tagged the version of our code we used to run the analysis.
Even if we continue to develop beyond commit 5 (above) after we submit our manuscript, we can always go back and run the analysis as it was in the past.
...
Multiple people can work on the same analysis at the same time on their own computer and everyone's changes can eventually be merged together.
...
the files.  Looks like `kellijohnson` was working on the project and fixing errors in December:

![](images/ss3sim-commits.png)

And finally, if we drill into one of the changes made on December 20, we can see exactly what
...
#### `Git` commands to use in the terminal {.unnumbered}
...
This is a regular Markdown file, so you can just add markdown text. When done, add a commit message, and hit the "Commit changes" button.  
...
messages.  In addition, the header above the file listing shows the most recent
commit, along with its commit message, and its SHA identifier.  That SHA identifier is
the key to this set of versioned changes.  If you click on the SHA identifier (*6c18e0a*), 
...
You'll note that there is one new file `training_test.Rproj`, and three files that we
...
In this case, the `.gitignore` file is listed as *M* for Modified, and `training_test.Rproj` 
...
Add a new section to your markdown using a header, and under it include a list of three advantages to using `Git`.
...
Note that some of the changes in the repository, namely `training_test.Rproj` are still listed as having not been committed.  This means
...
history of your repository meaningful and helpful.  It's tempting to skip the 
...
you did, but why you did it.  Also, commit messages are best understood if they
...
in a short phrase, for more complex changes, it's best to provide a more complete
...
been so grateful for the diligence of my past self and collaborators.

### Collaboration and conflict free workflows {.unnumbered}
...
That said, it's truly painless if you can avoid merge conflicts in the first place.
You can minimize conflicts by:

- Ensure that you pull down changes just before you commit
  + Ensure that you have the most recent changes
...
In the repository name field, enter the **same** name as your R Project. So for me, this would be `training_dolinh`. Add a description, keep the repository public, and, most importantly:
...
On your repository page, GitHub has a button that will help you add a `README.md` file. Click the "Add a README" button and use markdown syntax to create a simple `README.md` Commit the changes to your repository.
