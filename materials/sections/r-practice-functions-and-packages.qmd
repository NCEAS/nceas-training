...
b. Function calculates your age for the current year. **Hint**: You can access the current year using `format(Sys.Date(), "%Y")`. `Sys.Date()` returns the current date, and `format()` and `"%Y"` formats the date to just the year in the format `YYYY` (as opposed to `YY`).
...
::: {.callout-note}
#### Question 3
Use `usethis::use_testthat()` to test the age function.
:::

### Test, Check, and Install `aboutMe`

Use either the Build Tab or run the commands in the Console. 

After, go to the Packages Tab in the Files Pane, search for the `aboutMe` package, and check that the documentation for your package looks as you expected. Try loading the package using `library()` and call some of the functions you created.
