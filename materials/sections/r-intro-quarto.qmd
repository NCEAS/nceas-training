...
literate analysis with Quarto/RMarkdown?? can look like using a real example.
[Here is an
...
hand, am RMarkdown file has a `.Rmd` extension. Both have similar
...
::: callout-important
One important difference between Quarto documents and RMarkdown
...
It is important to emphasize one more time that in an Quarto (and RMarkdown) document, the gray areas of the document are *code*, in this
...
Quarto has a "what you see is what you mean" (WYSIWYM) editor or Visual
...
many, many other contexts besides Quarto and RMarkdown. For example, formatting GitHub comments and README files.
...
Quarto document (`bibliography: references.bib`). Adding another
...
<!-- need to change 6 back to `r round(ratio)`-->
...
-   Run the last chunk in your Quarto document by pressing the play button on
    the chunk
:::
...
primary Quarto document, the authors chose to write one R script per file and
