...
[`qualtRics`](https://github.com/ropensci/qualtRics) (note the capital R) is an R package that enables retrieval of Qualtrics data directly from the Qualtrics platform and into R, reducing pre-processing time associated with data manipulation and import. Although Qualtrics is proprietary software, the `qualtRics` R package is open source and community developed. The only limitation to this package is that your institution must support Qualtrics API access and it must be enabled in your account to use it.
