...
As we mentioned yesterday, DayMet is a gridded weather data product. It comes  The first step is to 
...
The AZMet data format is described here: https://cals.arizona.edu/azmet/raw2003.htm
...
**Slope** and plots of model vs obs with a 1:1 line 
**RMSE** Root Mean Square Error

**R^2** 
**Bias**: does one constantly over predict? $\bar{Y_{model}}-\bar{Y_{obs}}
**Ratio of variances** $\frac{SD_{Y_{model}}}{SD_{Y_{obs}}}$

#### Slope

of regression of predicted on observed: $Y_{obs}= bY_{mod}+\epsilon$. Does slope $b=1$?
...
* $RMSE/sd(obs)$
* $RMSE/\bar{Y_{obs}}$ equivalent to coefficient of variance $\frac{\sigma}{\bar{x}}$
* $RMSE/IQR$

#### $R^2$ Corellation Coefficient

Correlation Coefficient in terms of deviation from the 1:1 line
...
<details>
must be a strange scaling algorithm
