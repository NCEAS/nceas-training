...
1. Add a new R Markdown file at the top level called `index.Rmd`
    a. Go to the RStudio menu  File > New File > R Markdown
...
2. Save the R Markdown file you just created. Use `index.Rmd` as the file name
...
3. Press "Knit" and observe the rendered output
    a. Notice the new file in the same directory `index.html`
    b. This is our R Markdown file rendered as `HTML` (a web page)
    
4. Commit your changes (for both `index.Rmd` and `index.html`) with a commit message, and `push` to GitHub
...
Next, we will show how you can link different `Rmd`s rendered into `html` so you can easily share different parts of your work. 
