...
```{r}
ggplot(survey_joined, aes(x = Q1, fill = "blue")) +
    geom_bar()
```
...
```{r}
ggplot(survey_joined, aes(x = Q1, fill = location)) +
    geom_bar()
```
If we want to make our bar chart side by side as opposed to stacked, we can set the `position` argument within `geom_bar` to dodge
...
Let's look at an example of a `theme` call, where we change the rotation of our x tick labels so that we can read them all more easily.
