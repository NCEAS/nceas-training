...
## Example 1
 Suppose you have the following `data.frame` called `length_data` with data about salmon lenght and want to calculate the average length per year. 
...
In this lesson we're going to walk through examples as the previous ones to learn the functions you'll most commonly use from the `dplyr` and `tidyr` packages:
