...
Warnings are important, but we might not want them in our final document. After you have read the packages in, adjust the chunk settings on your library chunk to suppress warnings and messages.
:::
...
Say you want to both `filter()` out rows of a data set, and `select()` certain columns. Instead of writing
