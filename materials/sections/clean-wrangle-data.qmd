...
- **Step 1:**  Navigate to the salmon catch data set: <https://knb.ecoinformatics.org/#view/df35b.304.2>

- **Step 2:** Select "Copy Link Address" from the dropdown menu

- **Step 3:** Paste the URL into the `read_csv()` function
...
**Note ti=o Windows users** Keep in mind, if you want to replicate this workflow in your local computer you also need to use the `url()` function here with the argument `method = "libcurl"`. It would look like this:
...
## Explore data
Similar to what we did in our Intro to Rmd lesson, it is good practice to skim through the data you just read. This is important to make sure the data is read as you were expecting and also it allows you to familiarize with the data.
...
Let's fix it by incorporating the `ifelse()` function to our `mutate()` call, which will change the value of the `Chinook` column to 1 if the value is equal to `I`, then will use `as.numeric()` to turn the character representations of numbers into numeric typed values.
...
catch_long <- catch_clean %>% 
  pivot_longer(cols = -c(Region, Year), #pivot longer all columns except Region and Year
               names_to = "species", 
               values_to = "catch")
