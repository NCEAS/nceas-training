...
Relative paths start from some location in your file system that is below the root. Relative paths are combined with the path of that location to locate files on your system. Often we refer to the location where the relative path starts as our *working directory*. 
...
When starting a new research project, one of the first things I do is set up an R project for it (just like we have here!) The next step is to then populate that project with relevant directories. There are many tools out there that can do this automatically. Some examples are [`rrtools`](https://github.com/benmarwick/rrtools) for research compendiums, or `usethis::create_package()` for packages, but for now we are going to set up some directories manually.

Currently, the only file in your project is your `.Rproj` file. This file is what contains the settings specific to your R project. We haven't changed anything here yet, but will when we learn git. As part of this course, we know we will be working with R scripts and data, so let's create a directory for each.

* select the "New Folder" button in your file browser window, while the window is showing the top level of your R project
* name the folder `R`
* click "ok"
* repeat for `data`

If we were writing a paper from this analysis, there would be other directories we could probably create (eg: figures, manuscript), but let's leave it at this for now.
