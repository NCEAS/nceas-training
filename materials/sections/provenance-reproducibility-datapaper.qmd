...
To capture the R packages and versions this project depends on, the `renv` package is used. 
- Running `renv::init()`, will initiate tracking of the R packages in your project.
This unction will create a new folder called `renv` in your top directory.
...
- `renv` also write the package dependencies to a special file in the repository called `renv.lock`. 
 ...
 - If any of your packages you are using get updated, while your are working on your project, you can run `renv::snapshot()` to update the `renv.lock` file and your project-installed packages. 
