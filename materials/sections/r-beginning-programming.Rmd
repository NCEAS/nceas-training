...
***Quick Tip:** you can change the default location of the panes, among many other things: [Customizing RStudio](https://support.rstudio.com/hc/en-us/articles/200549016-Customizing-RStudio).*
...
Let's say the value of 12 that we got from running `3*4` is a really important value we need to keep. The way we keep information in R is through **objects**. Objects are how we store information in R.
...
Assign `3*4` to an object and then inspect the object you just created.

***Quick Tip:** Use the up and down arrow keys to call your command history, with the most recent commands first*
...
```{r, eval=FALSE}
important_value            # printing the object by calling the object name
(important_value <- 3*4)   # printing the object by wrapping the assignment syntax or                              just the object name in paraentheses
```

#### Naming Conventions

Before we go into some more complex calculations, let's talk about naming objects. For the object, `important_value` I used underscores to separate the object name. This naming convention is called snake case. There are other naming conventions use including, but not limited to:
...
-   SomeUseUpperCamelCaseOrPascalCase

Choosing a [naming convention](https://en.wikipedia.org/wiki/Naming_convention_(programming)#:~:text=In%20computer%20programming%2C%20a%20naming,in%20source%20code%20and%20documentation) is up to personal preference, but once you choose one - be consistent! A consistent naming convention will increase the readability of your code for others and your future self. Note that object names cannot start with a digit and cannot contain certain other characters such as a comma or a space.

#### R calculations with objects

Now that we know what an object in R is and how to create one. Let's learn how to use an object in calculations. Create the object `weight_kg` and assign it a value of 55.

```{r, eval=FALSE, purl=FALSE}
weight_kg <- 55 
```

Now that R has `weight_kg` saved in the Global Environment, we can do arithmetic with it. For instance, we may want to convert this weight into pounds (weight in pounds is 2.2 times the weight in kg):
...
A vector is a common and important
...
Above we created both numeric and integer objects, where the
