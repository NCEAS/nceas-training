...
First let's create a *vector* of weights in kilograms using `c()`. 
...
Wouldn't it be helpful if we could save these new weights in pounds? This might be important information we may need to use for a future calculation. How would you save these new weights in pounds? 
...
Let's talk more about vectors. 

**A vector is the most common and most basic data structure in R**. The vector we will interact with the most during this course are **atomic vectors**. To put it simply, atomic vectors only contain elements of the *same* data type. 
...
So far we've been running code in the console, let's try running code in an R script. 
...
Your RStudio should open your R script automatically after creating it. Notice a new pane appears above the console. This is called the **source pane** and is where we will write and edit R code and documents. This pane is only present if there are files open in the editor.
...
Now that we know what an object is in R and how to create one let's learn how to use an object in calculations. So far we've used R to run some basic calculations, why don't we try a complex calculation using a the function `mean()` (don't worry we'll discuss functions in more depth below).

```{r, eval=FALSE}
# take the mean of a set of numbers
mean(c(4, 5, 10, 2))
# create an object using the vector of numbers
my_numbers <- c(4, 5, 10, 2)
# call the object `my_numbers`
my_numbers
# take the mean of `my_numbers`
mean(my_numbers)
