...
Since we just cleared our environment, let's recreate our weights object again. This time let's say we have three dog weights:
...
Let's say our dogs gained 5 pounds and we need to update our vector, so let's change our object's value by assigning it new and update values.
...
It's important to understand how an R script run - which is top to bottom. This order of operations is important because if you are running code line by line, the values in object may be unexpected. When you are done with an R script, it's good practice to clear your Global Environment and use the `Run` button and select "Run all" to test that your script successfully runs top to bottom.
