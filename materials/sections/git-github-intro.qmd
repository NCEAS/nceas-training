...
#### What is the difference between "normal" folder vs. a `Git` repository
...
- Stage the file(s) using `git add myAnalysis.R` (or `git add .` to add multiple changed files at once). This lets Git know that you'd like to include the file(s) in your next commit
...
The last step is synchronizing the changes made to our local repository with a remote repository (oftentimes, this remote repository is stored on GitHub). The `git push` command is used to send local changes up to a remote repository. The `git pull` command is used to fetch changes from a remote repository and merge them into the local respoitory (e.g. **pull**ing will become a regular part of your workflow when collaborating with others, or even when working alone but on different machines, e.g. a laptop at home and a desktop at the office).
