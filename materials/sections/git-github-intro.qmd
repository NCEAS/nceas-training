...
There are a number of different workflows for creating version-controlled repositories that are stored on GitHub. We started with [Exercise 1](https://learning.nceas.ucsb.edu/2023-09-ucsb-faculty/session_09.html#exercise-1-create-a-remote-repository-on-github) and [Exercise 2](http://localhost:5456/session_09.html#exercise-2-clone-your-repository-and-use-git-locally-in-rstudio) using one common approach: **creating a remote repository on GitHub first, *then* cloning that repository to your local computer (you used your `{FIRSTNAME}_test` repo).**
...
1. **Initialize `{USERNAME}_training` as a Git repository** by running `git init` in the Terminal. You should get a message that says something like:
...
# this sets the default branch name to `main` for any new repos movign forward
...
5. **Create an empty remote repository** by logging into [GitHub](https://github.com/) and creating a new repository, following the same steps as in [Exercise 1](http://localhost:5456/session_09.html#exercise-1-create-a-remote-repository-on-github). **IMPORTANTLY,** *DO NOT* initialize your remote repo with a `README` license, or `.gitignore` file -- doing so now can lead to merge conflicts. We can add them after our local and remote repos are linked. **Name your remote repository the same as your local repository (i.e. `training_{USERNAME}`).**
...
#| fig-align: "center"
#| out-width: "100%"
knitr::include_graphics("images/setup-empty-repo-sam.png")
```
