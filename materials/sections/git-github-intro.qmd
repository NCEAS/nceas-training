...
This tells us that we have 1 commit in the local repository, but that commit has not yet been pushed up to the `origin` repository (aka remote repository on GitHub). Let's commit the remaining project files by staging/adding them and commiting them with an informative commit message.
