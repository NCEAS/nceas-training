...
- **Step 1:** Load the packages we'll need:
...
- **Step 2:** Load the data table directly from the KNB Data Repository, if it isn't already present on your local computer.  This technique only downloads the file if you need it.
...
- **Step 3:** Learn about the data. For this session we are going to be working with data on _daily salmon escapement counts in Alaska_. Check out the documentation [here](https://knb.ecoinformatics.org/view/doi%3A10.5063%2FP26WJJ).
...
It is more frequent than not, that we need to do some wrangling before we can plot our data the way we want to. Now that we have read out data and have done some exploration, we'll put our data wrangling skills to practice to get our data in the desire format.
...
Next, we will use the pipe operator to pass into `ggplot` a filtered version of `annual_esc`, and make a plots with different geometries.
...
- If you are want to update your plot base on a constant  (e.g. “Make ALL the points BLUE”), you can add the information directly to the relevant geom_ layer.
...
We know that in the graph we just plotted, each bar includes escapements for multiple years. Let's leverage on the power of `ggplot` to plot more aspect of our data in one plot.
...
Use the RStudio auto complete, the `ggplot2` documentation, a cheatsheet, or good old Google to find other built in themes. Pick out your favorite one and add it to your plot.
...
2. What changes do you expect to see in you plot by adding the following line of code? Discuss with your neighbor and then try it out!
...
Now, let's apply some of the customizations we have seen so far and learned a some new ones.
...
## Publish the Data Vis lesson to your webpage

::: callout-tip
## Steps

1. Save the Rmd you have been working on for  this lesson.
2. Knit the Rmd. This is a good way to test everything in your code is working.
3. Go to your `index.Rmd` and and the link to the html file with with lesson's content.
4. Save and render `index.Rmd`
