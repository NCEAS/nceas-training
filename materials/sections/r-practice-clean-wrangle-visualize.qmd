...
2.  **Collaborator** creates new files for exercise
    a. The Collaborator creates the following directory:
        i. `analysis` 
    b. After creating the directories, create the following Quarto files and store them in the listed folders:
    ...
    c.  The Owner `pull`s the changes and Quarto files into their local repository (their workspace)
