...
2.  **Collaborator** creates new files for exercise
    a. The collaborator creates the following directories:
        i. `analysis` 
...
3.  **Owner** downloads data from the EDI Data Portal [SBC LTER: Reef: Abundance, size and fishing effort for California Spiny Lobster (Panulirus interruptus), ongoing since 2012](https://portal.edirepository.org/nis/mapbrowse?packageid=knb-lter-sbc.77.8).
    ...
    c.  After creating the `data` folder and adding the data, the owner will `stage`, `commit`, write a commit message, and `push` the files to the remote repository (on github)
    d.  The collaborator pulls the changes and data into their local repository (their workspace)
:::
...
Questions 1-5 you will be working independently since you're working with different data frames, but you're welcome to check in with each other.
