...
Navigate to the demo data package here [https://test.arcticdata.io/view/urn:uuid:b8570c6f-e8c9-4e0e-ae7b-ee55faca5763](https://test.arcticdata.io/view/urn:uuid:b8570c6f-e8c9-4e0e-ae7b-ee55faca5763) and select "Download All". Unzip the downloaded file and navigate to `strix-pacitic-northwest.xml` within the `data` directory. This is the metadata file associated with this data package.
...
library(EML) # requires package EML_2.0.0
# Load the EML file into R
emlFile <- "data/strix/strix-pacific-northwest.xml"
doc <- read_eml(emlFile)
# Change creator to us
me <- list(individualName = list(givenName = "Matthew", surName = "Jones"), 
           electronicMailAddress = "jones@nceas.ucsb.edu")
doc$dataset$creator <- me
# Change abstract to the better one we wrote
doc$dataset$abstract <- set_TextType("data/strix/better-abstract.md")
