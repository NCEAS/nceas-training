...
The *datapack* R package represents the set of files in a dataset as a `datapack::DataPackage`. This `DataPackage` is just a special R object class that is specified in the `datapack` package. More broadly, remember that a data package consists of all of the objects (or files) in a dataset, such as scripts, tabular data files, and images, in addition to the metadata file that describes these files. `datapack` has some special ways to bundle all of these files in a way that makes sense, but from a computational perspective, every file associated with a data package (including the metadata file) is considered a "data object" in the package.  We will create a `DataPackage` locally, and then upload it to a test version of the Arctic Data Center repository using `dataone`.
...
```{r}
d1c <- D1Client(cn = "STAGING", mn = "urn:node:mnTestARCTIC")
mn <- d1c@mn
```
...
We then need to add a metadata file and data file to this package. First we generate some identifiers for the objects:
...
First, let's create a new `DataObject`, which is another object class specific to `datapack`. Confusingly, our metadata file, data file, and script will all need to be added as a `DataObject`. Remember that technically all files in a package are considered data objects, not just the ones that we might traditionally think of as being "data".
...
We add the data file and the script similarly. The only difference is in the `addMember` function we have to set the `mo` (metadata object) argument equal to the `DataObject` that represents the metadata file.
...
You can also add provenance information that indicates the relationship between the CSV and the R script using the function `describeWorkflow`
