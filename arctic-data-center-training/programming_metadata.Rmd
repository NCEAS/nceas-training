...
For datasets with a relatively small number of files that do not have many columns, the Arctic Data Center web form is the most efficient way to create metadata. However, for datasets that have many hundreds of files with a similar structure, or a large number of attributes, programmatic metadata creation may be more efficent.
...
EML is written in XML (extendable markup language). One of the key features of XML is that it has elements. An XML element is everything that is encompassed by a start tag (`<...>`) and and end tag (`</...>`). So, a simple element in the example above is `<surName>Willette</surName>`. The name of this element is `surName` and the value is simple text, "Willette". Each element in EML has a specific definition that is specified in the schema. The schema specifies not only the definitions of all EML elements, but also certain requirements, like which elements must be included, what type of values are allowed within the elements, and how the elements are organized.

You'll notice that some elements, rather than having a simple value, instead contain other elements that are nexted within them. Let's look at `individualName`.
...
This element is a collection of other elements (sometimes referred to as child elements). In this example, it has a `givenName` and a `surName` child element. We can check the requirements of any particular element by looking at the [schema documents](https://eml.ecoinformatics.org/schema/).
...
This shows that within `individualName` there are 3 child elements: `salutation`, `givenName`, and `surName`. The yellow circle icon with stacked papers tells you that the elements come in series, so you can include one or more of the child elements (as opposed to a switch symbol, which means that  you choose one element from a list of options). The bold line tells you that `surName` is required.

So, to summarize, EML is written in XML. It consists of a series of nested elements. The slement definitions, required elements, and structure are all defined by a schema. When you write EML, in order for it to be valid, your EML document must conform to the requirements given in the schema. 

### Metadata in R: a simple example

Now, let's learn how the [EML package](https://github.com/ropensci/EML) can help us create EML.

First, load the EML package.
...
At the top level of a complete EML document are the `packageId` and `system` elements. This is how your document can be uniquely identified within whatever system manages it. Nested within the top level is the `dataset` element. All EML documents must have, at a minimum, a `title`, `creator`, and `contact`, in addition to the `packageId` and `system`.
...
First we need to create an R object that describes where we will be uploading our dataset. We do this with the `dataone` function `D1Client` (DataONE client). The arguments for this function are a little opaque, but they basically describe the DataONE coordinating node (in this case a test node called STAGING) and the DataONE member node.

We'll also create an object that only references the member node, which is helpful for some functions.
...
Next, we create a `DataPackage` as a container for our data, metadata, and scripts.
...
We then add a metadata file and data file to this package. First we generate some identifiers for the objects:
...
Now we need to modify our EML document to include these identifiers. First read in the EML.
...
Now let's put the data object identifier in the `dataTable` that we created as a distribution URL. All of the distribution URLs look something like this `https://test.arcticdata.io/metacat/d1/mn/v2/object/urn:uuid:A0cc95b46-a318-4dd1-8f8a-0e6c280e1d95`, where `https://test.arcticdata.io/metacat/d1/mn/v2/` is the member node end point, and `urn:uuid:A0cc95b46-a318-4dd1-8f8a-0e6c280e1d95` is the identifier.
...
Now we have a full metadata document ready to be uploaded. Now we can add our files to the `DataPackage`.

First, let's create a new `DataObject`, which is a type of object class supported by `datapack`. Confusingly, both our metadata file and our data file will need to be added as a `DataObject`. In this case, `DataObject` just means any type of file in the data package, so that would include data files, the metadata files, scripts, figures, etc.
...
We add the data file and the script similarly. The only difference is in the `addMember` function we have to set the `mo` (metadata object) argument to the `DataObject` that represents the metadata file.
...
You can give your colleague access to edit the data package using `addAccessRule`.
...
You can also add provenance information that indicates the relationship between the csv and the R script.
...
Finally, we upload the package to the test server for the Arctic Data Center.
