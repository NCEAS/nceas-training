...
At the top level of a complete EML document are the `packageId` and `system` elements. This is how your document can be uniquely identified within whatever system manages it,and typically contain the DOI (Digital Object Identifier) or other identifier for the dataset. Nested within the top level is the `dataset` element. All EML documents must have, at a minimum, a `title`, `creator`, and `contact`, in addition to the `packageId` and `system`.
